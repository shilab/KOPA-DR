...
\mathcal{L}*{\mathrm{LLM}}
+
\lambda*{\mathrm{DDI}}
\mathcal{L}_{\mathrm{DDI}}
]
...
and the DDI-aware safety weight is:

[
w_{\mathrm{DDI}}(i,j)
=====================

\alpha , w_{\mathrm{coT}}(i,j)
+
\beta , \phi_{\mathrm{ADR}}(i,j)
+
\gamma , w_{\mathrm{topo}}(i,j)
]
...
```bash
KoPA-DR/
├── kopaddi.py                     # Core KoPA-DR method
├── finetune_kopa_ddi.py           # DDI-aware training
├── evaluate_kopa_ddi.py           # DDI-aware evaluation
├── rotate.py                      # RotatE-based graph encoding
├── process_kge.py                 # KGE preprocessing
├── data/
   └── sample/                    # Sample PrimeKG datasets
```
